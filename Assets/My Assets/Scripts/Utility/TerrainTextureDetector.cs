...
{
    public enum TerrainTextureName
    {
        None,
        Grass,
        Rock,
    ...
    {
        [SerializeField]
        private List<TerrainTextureName> _textureNames;
        ...
        private void Start()
        {
            ...
            CachedTerrainAlphamapData =
                ThisTerrainData.GetAlphamaps(0, 0, ThisTerrainData.alphamapWidth, ThisTerrainData.alphamapHeight);
        }

        public TerrainTextureName GetDominantTextureNameAt(Vector3 worldPosition)
        {
            var textureIndex = GetDominantTextureIndexAt(worldPosition);

            if (textureIndex < 0)
            {
                return TerrainTextureName.None;
            }

            if (textureIndex <= _textureNames.Count - 1)
            {
                return _textureNames[textureIndex];
            }
